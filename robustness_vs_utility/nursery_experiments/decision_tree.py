...
def hyperparameter_tuning():
    ...
    param_dist = dict(max_depth=max_depth_range, criterion=criterion_range)
...
def demo_exp(gamma, n_removed_attr):  # [20, 10, 7, 3]
    ...
    accuracy_original = 0.7841820987654321  # {'criterion': 'entropy', 'max_depth': 22}
    ...
    for attr_combination in attributes:
        ...
        for n in range(n_exp):
            ...
            if attr_combination[0] is 'full':
                ...
                model = DecisionTreeClassifier(random_state=random_state, criterion='entropy', max_depth=22)
                ...
                model2 = DecisionTreeClassifier(random_state=random_state, criterion='entropy', max_depth=22)
...
def score_on_diff_target(model, features, target):
...
def feature_selection(gamma):
    ...
    for e in range(n_exp):
        ...
        for n_features in range(1, len(fp_dataset.columns)):
            ...
            model = DecisionTreeClassifier(random_state=0, criterion='entropy', max_depth=22)
            s = cross_val_score(model, features, target, cv=10)
            #print(np.mean(s))
            scores[n_features].append(np.mean(s))
            # same model; features - new_target + old_target; new_target
            new_target_name = 'health'
            new_target = fp_dataset[new_target_name].values
            new_model = DecisionTreeClassifier(random_state=0, criterion='gini', max_depth=6)
            if new_target_name not in features_names:
                scores_on_different_target[n_features].append(score_on_diff_target(new_model, features, new_target))
            else:
                print("Warning: the new target 'health' is in the feature list. Using one feature less.")
                support_new = feature_model.get_support() != np.array([col == 'health' for col in fp_dataset.columns])
                features_names_new = fp_dataset.columns.values[support_new]
                features_new = fp_dataset[features_names_new]
                try:
                    scores_on_different_target[n_features].append(score_on_diff_target(new_model, features_new,
                                                                                       new_target))
                except ValueError:
                    pass
        secret_key += 2

    return scores, scores_on_different_target
...
def transferable_feature_selection(gamma):
    ...
    for e in range(n_exp):
        ...
        for n_features in range(1, len(fp_dataset.columns)):
            ...
            new_model = DecisionTreeClassifier(random_state=0, criterion='gini', max_depth=7)
            # define new target
        ...
        secret_key += 2
    return scores_on_different_target
...
def horizontal_attack(gamma, percentage_removed, results):
    n_exp = 7
    # LOAD DATA
    data = pd.read_csv('datasets/nurser y_full.csv', index_col='Id')
    target = data['target']
...
def bit_flipping_attack(gamma, percentage_flip, results):
    ...
    for e in range(n_exp):
        ...
        model = DecisionTreeClassifier(random_state=0, criterion='entropy', max_depth=9)
...
if __name__ == '__main__':
    ...
    # save results
    outfile = open('robustness_vs_utility/nursery_experiments/horizontal_with_imputation_decision_tree_' + str(gamma), 'wb')
    pickle.dump(results, outfile)
    outfile.close()
    #score_target_health = transferable_feature_selection(gamma)
#    pprint(score, score_target_1)
    #outfile = open('robustness_vs_utility/nursery_experiments/transferable_feature_selection_score_parents_' + str(gamma),
    #               'wb')
    #pickle.dump(score_target_health, outfile)
    #outfile.close()
