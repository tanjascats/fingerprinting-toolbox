...
def hyperparameter_tuning():
    ...
    param_dist = dict(n_estimators=n_estimators_range, criterion=criterion_range)
    # hyperparameter random search
    # take the best accuracy from 10-fold cross validation as a benchmark performance
    model = GradientBoostingClassifier()
    rand_search = RandomizedSearchCV(model, param_dist, n_iter=10, cv=10, scoring="accuracy", random_state=0)
    rand_search.fit(data_dumm, target)
    best_params = rand_search.best_params_
    print(best_params)
    print(rand_search.best_score_)
    print(rand_search.best_estimator_)
    outfile = open('robustness_vs_utility/nursery_experiments/gradient_boosting_hyperparameters', 'wb')
...
def demo_exp(gamma, n_removed_attr):  # [20, 10, 7, 3]
    ...
    with open("robustness_vs_utility/nursery_experiments/log_formatted", "a+") as log_formatted:
        ...
        gamma_formatted = {5: 0, 10: 1, 20: 2, 30: 3}
...
def feature_selection(gamma):
    ...
    for e in range(n_exp):
        ...
        for n_features in range(1, len(fp_dataset.columns)):
            ...
            model = GradientBoostingClassifier(random_state=0, criterion='friedman_mse', n_estimators=100,
                                               loss='deviance')
...
def horizontal_attack(gamma, percentage_removed, results):
    ...
    for e in range(n_exp):
        ...
        model = GradientBoostingClassifier(random_state=0, criterion='friedman_mse', n_estimators=100, loss='deviance')
...
def transferable_feature_selection(gamma):
    ...
    for e in range(n_exp):
        ...
        for n_features in range(1, len(fp_dataset.columns)):
            ...
            new_model = LogisticRegression(C=10, solver='liblinear')
            # define new target
...
if __name__ == '__main__':
    ...
    # save results
    outfile = open('robustness_vs_utility/nursery_experiments/horizontal_with_imputation_gradient_boosting_' + str(gamma),
                   'wb')
