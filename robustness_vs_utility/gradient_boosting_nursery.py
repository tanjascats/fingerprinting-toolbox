...
# nursery dataset
nursery, pk = import_dataset('nursery_full')
# preprocess
nursery = nursery.drop('Id', axis=1)
# separate target from the data
c = len(nursery.columns)
target = nursery.values[:, (c-1)]
le = LabelEncoder()
target = le.fit_transform(target)

nursery = nursery.drop("target", axis=1)
...
#param_dist = dict(criterion=criterion_range, n_estimators=n_estimators_range)
...
print("\t-best parameters: {'n_estimators':100, 'loss':'deviance', 'criterion':'friedman_mse'}")
print("\t-best accuracy score: 0.9708")
...
for n in range(n_exp):
    ...
    model2 = GradientBoostingClassifier(random_state=random_state, n_estimators=100, loss='deviance',
    ...
    for remove in removes:
        ...
        for attribute_combination in combinations_to_consider:
            ...
            model3 = GradientBoostingClassifier(random_state=random_state, n_estimators=100, loss='deviance',
                                        criterion='friedman_mse')
