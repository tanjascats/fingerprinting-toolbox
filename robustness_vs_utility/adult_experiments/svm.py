...
def hyperparameter_tuning():
    ...
    model = SVC()
    rand_search = RandomizedSearchCV(model, param_dist, n_iter=10, cv=10, scoring="accuracy")
...
def demo_exp(gamma, n_removed_attr):  # [20, 10, 7, 3]
    ...
    f = open("robustness_vs_utility/adult_full/log.txt", "a+")
    pprint(score, f)
    pprint("Averages", f)
    ...
    with open("robustness_vs_utility/adult_full/log_refined.txt", "a+") as log_refined:
...
if __name__ == '__main__':
    ...
    for portion in range(0, 60, 5):
        ...
        print(str(portion) + ' out of ' + str(60) + " in time: " + str(int(time.time() - start)))
    # save results
    outfile = open('robustness_vs_utility/adult_experiments/bit-flip_svm_' + str(gamma), 'wb')
