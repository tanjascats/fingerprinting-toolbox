...
# 0) import data
nursery, primary_key = import_dataset("nursery_full")
...
for n in range(n_exp):
    ...
    if not fp_dataset.columns.equals(nursery.columns):
        difference = nursery.columns.difference(fp_dataset.columns)
        for diff in difference:
            fp_dataset[diff] = np.zeros((len(fp_dataset),), dtype=np.int)
    fp_dataset = fp_dataset[nursery.columns.tolist()]
