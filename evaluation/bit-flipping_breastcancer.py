...
def flipping_attack(overwrite_existing=False): # prerequisite is that the fingerprinted datasets are available fingerprinted_data/breastcancer
    ...
    all_experiment_results = os.listdir('robustness/flipping/breastcancer')
    existing_results = []
    ...
    for fp_dataset_path in all_fp_datasets:
        ...
        if xi != 1: continue  # categorical; xi makes no difference
...
def flipping_false_miss_estimation():
    ...
    for fp_len in parameter_grid['fp_len']:
        for gamma in parameter_grid['gamma']:
            ...
            with open('robustness/flipping_est/breastcancer/false_miss_l{}_g{}_x1.json'.format(fp_len, gamma),
                      'w') as outfile:
                json.dump(false_miss, outfile)
