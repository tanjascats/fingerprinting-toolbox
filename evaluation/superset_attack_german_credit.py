...
def superset_attack(overwrite_existing=False): # prerequisite is that the fingerprinted datasets are available fingerprinted_data/german_credit
    # read existing experiments
    all_experiment_results = os.listdir('robustness/superset/german_credit')
    existing_results = []
    ...
    for fp_dataset_path in all_fp_datasets:
        ...
        if xi == 2 or xi == 4: continue # skip multiple values for xi because xi does not affect the superset attack
        ...
        with open('robustness/superset/german_credit/false_miss_l{}_g{}_x{}.json'.format(fp_len, gamma, xi), 'w') as outfile:
            json.dump(false_miss, outfile)
        modified_files.append('robustness/superset/german_credit/false_miss_l{}_g{}_x{}.json'.format(fp_len, gamma, xi))
        with open('robustness/superset/german_credit/misattribution_l{}_g{}_x{}.json'.format(fp_len, gamma, xi), 'w') as outfile:
            json.dump(misattribution, outfile)
        modified_files.append('robustness/superset/german_credit/misattribution_l{}_g{}_x{}.json'.format(fp_len, gamma, xi))
