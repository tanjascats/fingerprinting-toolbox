...
def superset_attack(): # prerequisite is that the fingerprinted datasets are available fingerprinted_data/breast_cancer_w
    ...
    for fp_dataset_path in all_fp_datasets:
        ...
        with open('robustness/superset/breast_cancer_w/false_miss_l{}_g{}_x{}.json'.format(fp_len, gamma, xi), 'w') as outfile:
            json.dump(false_miss, outfile)
        with open('robustness/superset/breast_cancer_w/misattribution_l{}_g{}_x{}.json'.format(fp_len, gamma, xi), 'w') as outfile:
...
def superset_check():
    ...
    attacked_fp_dataset = attack.run(dataset=fp_dataset.dataframe,
                                     primary_key_attribute=fp_dataset.primary_key_attribute, strength=0.6,
    ...
    for fp_len in parameter_grid['fp_len']:
        for gamma in parameter_grid['gamma']:
            ...
            for strength in np.arange(0.0, 1.1, 0.1):
                attack = attacks.Superset()
                false_miss[strength] = attack.false_miss_estimation(dataset=dataset, strength=strength,
                                                                    scheme=scheme)
            with open(
                    'robustness/superset_pure_est/breast_cancer_w/false_miss_l{}_g{}_x1.json'.format(fp_len, gamma),
                    'w') as outfile:
                json.dump(false_miss, outfile)


def main():
    superset_estimation_pure()
