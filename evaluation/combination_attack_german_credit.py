...
import time
from rounding_attack_german_credit import *
from superset_attack_german_credit import *
...
def combination_attack(overwrite_existing=False): # prerequisite is that the fingerprinted datasets are available fingerprinted_data/german_credit
    ...
    for fp_dataset_path in all_fp_datasets:
        ...
        for strength in strength_grid:
            ...
            # early stop criteria -- 1.0 for ascendoin
            if false_miss[strength] == 1.0:
                break
        print(false_miss)
        print(misattribution)
        with open('robustness/combination/german_credit/false_miss_l{}_g{}_x{}.json'.format(fp_len, gamma, xi), 'w') as outfile:
            json.dump(false_miss, outfile)
        modified_files.append('robustness/combination/german_credit/false_miss_l{}_g{}_x{}.json'.format(fp_len, gamma, xi))
        with open('robustness/combination/german_credit/misattribution_l{}_g{}_x{}.json'.format(fp_len, gamma, xi), 'w') as outfile:
            json.dump(misattribution, outfile)
        modified_files.append('robustness/combination/german_credit/misattribution_l{}_g{}_x{}.json'.format(fp_len, gamma, xi))
