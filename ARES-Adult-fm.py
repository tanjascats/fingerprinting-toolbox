...
def test_vertical():
    ...
    # 0.95->len(data.columns)-2 = 13
    # 0.9 ->len(data.columns)-3 = 12
    # 0.6 -> int(0.6*len(data.columns))
    # 0.3 -> int(0.3*len(data.columns))

    strength = int(0.6*len(data.columns))
    ...
    for exp in range(n_exp):
        ...
        attacked = attack.run_random(dataset=fp_data.dataframe, number_of_columns_to_del=strength, seed=exp,
                                     keep_columns=['income'])
        recipient = scheme.detection(attacked, secret_key=exp, target_attribute='income')
        if recipient == 1:
            success += 1
    print('gamma =', gamma)
...
def test_flipping():
    ...
    data = Adult()
    attack = FlippingAttack()
    strengths = [0.9]#,0.6,0.9,0.95]

    n_exp = 10

    success = {0.3:0 ,0.6:0, 0.9:0, 0.95:0}
    for exp in range(n_exp):
        start_iter = time.time()
        fp_data = scheme.insertion(dataset=data, secret_key=exp, recipient_id=1)
        for strength in strengths:
            attacked = attack.run(dataset=fp_data.dataframe, strength=strength, keep_columns=['income'],
                                  random_state=exp)
            recipient = scheme.detection(attacked, secret_key=exp, target_attribute='income')
            if recipient == 1:
                success[strength] += 1
        print(success)
        print('Time for iteration: ', int(time.time()-start_iter), 'seconds')
    print('gamma =', gamma)
    print('strength = ', strengths)
    fm = {strength: round(1.0 - success[strength] / n_exp, 2) for strength in strengths}
    print('fm =', fm)
