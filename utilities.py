...
def number_encode_features(ds):
    ...
    for feature in result.columns:
        if result.dtypes[feature] == np.object_:
...
def fp_fit_and_score(estimator, X_original, y_original, X_fingerprinted, y_fingerprinted, scorer, train_original,
    ...
    if verbose > 2:
        if split_progress is not None:
            progress_msg = " {split_progress[0]+1}/{split_progress[1]}"
        if candidate_progress and verbose > 9:
            progress_msg += ("; {candidate_progress[0]+1}/"
    ...
    if verbose > 1:
        if parameters is None:
            ...
            params_msg = (', '.join('{k}={parameters[k]}'
                                    for k in sorted_keys))
    if verbose > 9:
        start_msg = "[CV{progress_msg}] START {params_msg}"
        print("{start_msg}{(80 - len(start_msg)) * '.'}")
        ...
        if verbose > 2 and isinstance(test_scores, dict):
            for scorer_name in sorted(test_scores):
                ...
                if return_train_score:
                    ...
                    result_msg += "train={scorer_scores:.3f}, "
